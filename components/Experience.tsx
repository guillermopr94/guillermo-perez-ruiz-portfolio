...
const Experience: React.FC = () => {
  ...
  return (
    <section
      id="experience"
      className="py-20 bg-slate-950 relative scroll-mt-20"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="mt-4 text-slate-400 max-w-2xl mx-auto">
            A journey through technical leadership, legacy modernization, and
            scalable architecture. Click on a role to see more details.
          </p>
        </motion.div>
            ...
            {EXPERIENCE_DATA.map((job, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`flex-1 w-full ${isEven ? 'md:pr-12' : 'md:pl-12'}`}
                  >
                    <div
                      role="button"
                      tabIndex={0}
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="text-lg md:text-xl font-bold text-slate-100 group-hover:text-accent transition-colors">
                            {job.role}
                          </h3>
                          <div className="text-accent font-medium mb-1 text-sm md:text-base">
                            {job.company}
                          </div>
                        </div>
                        <ChevronDown
                            ...
                            <div className="pt-4 border-t border-slate-700/50">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {job.techStack.map((tech) => (
                                  <span
                                    key={tech}
                                    className="px-2 py-1 bg-slate-800/80 rounded text-[10px] md:text-xs text-accent border border-slate-700 font-mono"
                                  >
                                    {tech}
                                  </span>
                                ))}
            ...
            >
              ...
              <div className="w-full md:w-[350px] bg-slate-800/40 p-5 md:p-8 flex flex-col border-b md:border-b-0 md:border-r border-slate-800 shrink-0 overflow-y-auto">
                <div className="mb-4 md:mb-8">
                  <div className="text-accent text-[10px] md:text-xs font-bold tracking-widest uppercase mb-2">
                    Technical Deep Dive
                  </div>
                  <h3
                    id="experience-modal-title"
                    className="text-lg md:text-2xl font-bold text-white mb-1 leading-tight"
                  >
                    {modalJob.company}
                  </h3>
                  <div className="text-slate-400 text-[10px] md:text-sm">
                    {modalJob.role}
                  </div>
                </div>
                ...
                <div className="flex md:flex-col gap-2 md:gap-4 overflow-x-auto md:overflow-x-visible pb-2 md:pb-0 scrollbar-hide">
                  {modalJob.details?.map((detail, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`p-1.5 md:p-2 rounded-lg flex-shrink-0 ${isActive ? 'bg-slate-900/20' : 'bg-slate-800'}`}
                        >
                          <Icon className="w-3.5 h-3.5 md:w-5 md:h-5" />
                        </div>
                        <span className="font-bold text-[10px] md:text-sm line-clamp-1">
                          {detail.title}
                        </span>
                      </button>
                    );
                  ...
                  >
                    <div className="w-16 h-16 md:w-28 md:h-28 rounded-2xl md:rounded-[2rem] bg-accent/10 border border-accent/20 flex items-center justify-center mb-5 md:mb-10 shadow-[0_0_60px_rgba(56,189,248,0.15)] transform md:rotate-3">
                      {(() => {
                        const Icon =
                          iconMap[
                            modalJob.details?.[currentSlide].icon || 'Layers'
                          ] || Layers;
                        return (
                          <Icon className="w-8 h-8 md:w-14 md:h-14 text-accent" />
                        );
                      })()}
                    </div>
                  ...
                  <button
                    onClick={() =>
                      setCurrentSlide((prev) =>
                        Math.min((modalJob.details?.length || 1) - 1, prev + 1),
                      )
                    }
                    disabled={
                      currentSlide === (modalJob.details?.length || 1) - 1
                    }
